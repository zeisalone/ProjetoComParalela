...
// double PotentialPrepare(int N, double sigma, double * rx, double * ry, double * rz);
double Potential();
...
double Potential() {
    double r3, r2, r6, Pot;
    int i, j;
    double sig6 = power(sigma,6);
    Pot = 0.;
    for (i=0; i<N; i++) {
        for (j=0; j<N; j++) {
            if (j!=i) {

                r2  = power( (rx[i] - rx[j]), 2); 
                r2 += power( (ry[i] - ry[j]), 2); 
                r2 += power( (rz[i] - rz[j]), 2);
